...
As of 2026-07-31. Setup is complete; the project itself is undefined.
...
| Project | `C:\UE5\First` — Blank C++ template, Desktop, Scalable quality preset |
| Repo | `https://github.com/joehockey04/First` — private, `main` pushed |
...
`C:\UE5` is a projects root, not a repo. `C:\UE5\First` is the repo and the Unreal
...
- Package output goes outside `C:\UE5\First` — each build is several GB and Unreal
...
1. **What the project is.** Undecided. It is a portfolio piece for a co-op / internship
   application. Scope should be small, one interesting mechanic, finishable.
2. **Repo name.** `First` is a placeholder. Renaming the GitHub repo is trivial;
   renaming the Unreal project after assets exist is not — the `.uproject`, `Source/`
   folder, `.Build.cs`, module header/cpp, and the `FIRST_API` macro all carry the name.
   Decide before `Content/` fills up.
3. **Commit author email.** Both commits are authored `joseph.picard@picanne.com`.
   Kept deliberately. Revisit only if the repo goes public.
4. **Unreal MCP.** UE 5.8 ships an experimental first-party MCP plugin that lets an
